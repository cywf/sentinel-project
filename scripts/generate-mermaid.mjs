    ...
    for (const f of found) {
      ...
      for (const line of lines) {
        ...
        if (inServices) {
          ...
          // Only match keys at exactly baseIndent + 2 (direct children of services)
          if (indent === baseIndent + 2 && /^\s*([A-Za-z0-9._-]+)\s*:\s*$/.test(line)) {
...
async function buildCISequence() {
  ...
  for (const f of yamls.slice(0, 4)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inJobs) {
        // Look for 2-space indented keys (job names)
        const m = /^\s{2}([A-Za-z0-9_-]+):\s*$/.exec(line);
        if (m) {
          jobNames.push(m[1]);
        } else if (/^[A-Za-z]/.test(line) && line.trim()) {
          // New top-level key, exit jobs section
          break;
        }
      }
    }
    for (const j of jobNames.slice(0, 6)) {
      ...
      const jobBlock = (new RegExp(`\\b${j}:\\s*([\\s\\S]*?)(\\n\\s*[A-Za-z0-9_-]+:\\s*|\\n\\S|$)`, "m")).exec(text)?.[1] || "";
